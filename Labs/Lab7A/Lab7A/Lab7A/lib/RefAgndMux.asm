...
;  Global Symbols
;-----------------------------------------------
export  RefAgndMux_Start
export _RefAgndMux_Start

export  RefAgndMux_SetPower
export _RefAgndMux_SetPower

export  RefAgndMux_Stop
export _RefAgndMux_Stop

export  RefAgndMux_RefSelect
export _RefAgndMux_RefSelect
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RefAgndMux_Start
;  FUNCTION NAME: RefAgndMux_SetPower
;
;  DESCRIPTION:
...
_RefAgndMux_SetPower:
    ...
    mov  A, reg[RefAgndMux_REFMUX_CR2] ; read power value
    and  A, ~POWERMASK                 ; clear power bits in A
    or   A, [X]                        ; combine power value with balance of reg.
    mov  reg[RefAgndMux_REFMUX_CR2], A ; move complete value back to register
    pop  A
...
_RefAgndMux_Stop:
    ...
    and REG[RefAgndMux_REFMUX_CR2], ~POWERMASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RefAgndMux_InputSelect
;
;  DESCRIPTION:
...
;    functions.
;
 RefAgndMux_RefSelect:
_RefAgndMux_RefSelect:
    RAM_PROLOGUE RAM_USE_CLASS_2
    and  A,TMUX_MASK                   ; Mask off test mux control
    mov  X,SP                          ; Get stack location
    push A
    mov  A,reg[RefAgndMux_REFMUX_CR2]  ; Write register Test Mux register to select reference
    and  A, ~TMUX_MASK                 ; Clear Mux control bits
    or   A,[X]                         ; Or in the new setting
    mov  reg[RefAgndMux_REFMUX_CR2],A  ; Write register Test Mux register to select reference
